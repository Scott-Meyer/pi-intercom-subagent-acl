...
export function loadPersistedFederationOrigin(intercomDir: string): PersistedFederationOrigin | undefined {
  const path = join(intercomDir, FEDERATION_ORIGIN_FILE);
  if (!existsSync(path)) return undefined;
  try {
    const parsed: unknown = JSON.parse(readFileSync(path, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      console.warn(`pi-intercom: ${FEDERATION_ORIGIN_FILE} is corrupt; treating the federation origin as absent and adopting or minting a fresh identity.`);
      return undefined;
    }
    ...
    if (!isCanonicalFederationOriginId(record.originId)
      ...
      console.warn(`pi-intercom: ${FEDERATION_ORIGIN_FILE} has invalid contents; treating the federation origin as absent and adopting or minting a fresh identity.`);
      return undefined;
    }
    ...
    console.warn(`pi-intercom: ${FEDERATION_ORIGIN_FILE} could not be read; treating the federation origin as absent and adopting or minting a fresh identity.`);
    return undefined;
  }
}
...
export function persistFederationOrigin(
  ...
  restrictIntercomRuntimeFile(path, platform);
}
